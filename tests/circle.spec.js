...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    const circ = circle(2).circumference;
    expect(circ).toBeCloseTo(12.56);
    ...
    const radius = circle(3).radius;
    const circThree = circle(3).circumference;
    expect(area).toBeCloseTo(28.26);
    expect(radius).toBe(3);
    expect(circThree).toBeCloseTo(18.84);
  });
});
